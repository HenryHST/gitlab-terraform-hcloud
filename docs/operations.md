...
- **Docker-Image-Versionen:** `make check-images` vergleicht die gepinnten Tags **`gitlab_docker_gitlab_ce_image`**, **`gitlab_docker_traefik_image`** und **`gitlab_docker_postgres_image`** (Defaults in [`variables.tf`](../terraform/variables.tf), optional Override in `terraform.tfvars`) mit Docker Hub (`curl`, `jq` erforderlich). Für PostgreSQL wird der neueste Tag innerhalb der unterstützten Majors 13–17 geprüft. Tag-Listen werden in **`~/.docker-hub-cache/`** gecacht (TTL 1h, `CHECK_IMAGES_CACHE_DIR` / `CHECK_IMAGES_CACHE_TTL`). Exit-Codes: `0` = OK, `1` = veraltet (nur mit `make check-images-strict` / `--strict`), `2` = technischer Fehler. Nach einem Update: Variablen anpassen, `terraform plan`/`apply`, auf dem Host `docker compose pull` und betroffene Services neu starten. Nicht Teil von `make ci` (Netzwerk, Rate-Limits).
