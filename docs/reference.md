...
| `gitlab_docker_postgres_image` | `postgres:17-alpine` | PostgreSQL-Image (Major 13–17; Format `postgres:<major>[.<minor>[.<patch>]][-suffix]`); **18.x:** 16 oder 17; **19.x:** Terraform setzt automatisch **postgres:17** (Suffix aus dieser Variable, z. B. `-alpine`) — Output `gitlab_docker_postgres_image_effective`; `make check-images` prüft den neuesten Tag im unterstützten Bereich 13–17 |
...
| `gitlab_docker_traefik_hardening` | `{ enabled = false, … }` | Opt-in Traefik HTTP: **fail2ban**-Plugin (`default@file`), Log-Level (Standard `INFO`), Access-Log (`json`), **Rate-Limit** (100/s, Burst 200), TLS-Mindestversion (`VersionTLS12` / `VersionTLS13`) |
| `gitlab_docker_compose_hardening` | `{ enabled = false, … }` | Opt-in Docker/Compose: **`/etc/docker/daemon.json`** (`icc: false`, `live-restore`, `userland-proxy: false`, Log-Rotation), **`no-new-privileges`** und **json-file**-Logging für Stack-Services |
| `gitlab_docker_db_tuning` | `{ enabled = false, … }` | Opt-in DB: `gitlab_rails['db_pool']`, `sidekiq['concurrency']`; optional **PgBouncer** (`edoburu/pgbouncer`, Transaction-Pool) — GitLab → `pgbouncer:5432` → `postgres`; Backup/Restore direkt `postgres` |
